...
CONFIGS = [
    ...
    # Uniform baseline: FFN 28672/8=3584, q-heads 64/8=8, kv-heads 8/8=1 per rank.
    ("TP8PP1_uniform", 8, 1, [80],
     [3584] * 8, [8] * 8, [1] * 8),

    # FFN bias: Blackwell (rank 0-3) gets larger FFN slice, Ada (4-7) smaller.
    # +25% means Blackwell gets 25% more than uniform; check sum equals 28672.
    ("TP8PP1_ffn_bias+25",  8, 1, [80],
     [4480]*4 + [2688]*4, [8] * 8, [1] * 8),       # 17920 + 10752 = 28672
    ("TP8PP1_ffn_bias+50",  8, 1, [80],
     [5376]*4 + [1792]*4, [8] * 8, [1] * 8),       # 21504 +  7168
    ("TP8PP1_ffn_bias+75",  8, 1, [80],
     [6272]*4 +  [896]*4, [8] * 8, [1] * 8),       # 25088 +  3584
    ...
    # ---- TP1 PP8: 8 stages, 4 Blackwell + 4 Ada, no TP sharding ----
    # Per-stage TP=1 → whole-FFN/heads/kv per rank.
    ("TP1PP8_layer_uniform_10x8", 1, 8, [10]*8,
     [28672], [64], [8]),
    ("TP1PP8_layer_blackbias_12-12-12-12-8-8-8-8", 1, 8, [12,12,12,12,8,8,8,8],
     [28672], [64], [8]),
    ("TP1PP8_layer_blackbias_14-14-14-14-6-6-6-6", 1, 8, [14,14,14,14,6,6,6,6],
     [28672], [64], [8]),
]
