...
# OPT-30B: 48 layers, FFN 28672, 56 q-heads, 56 kv-heads (MHA, no GQA).
CONFIGS = [
    # ---- TP8 PP1: ffn 28672/8=3584, heads 56/8=7 each per rank
    ("TP8PP1_uniform", 8, 1, [48],
     [3584] * 8, [7] * 8, [7] * 8),
    # FFN bias only
    ("TP8PP1_ffn_bias+25",  8, 1, [48],
     [4480]*4 + [2688]*4, [7] * 8, [7] * 8),
    ("TP8PP1_ffn_bias+50",  8, 1, [48],
     [5376]*4 + [1792]*4, [7] * 8, [7] * 8),
    ("TP8PP1_ffn_bias+75",  8, 1, [48],
     [6272]*4 +  [896]*4, [7] * 8, [7] * 8),
    # Head bias only (q+kv same — MHA), Blackwell ranks 0-3 get more
    ("TP8PP1_head_bias+2",  8, 1, [48],
     [3584] * 8, [9]*4 + [5]*4, [9]*4 + [5]*4),   # 36+20=56
    ("TP8PP1_head_bias+3",  8, 1, [48],
     [3584] * 8, [10]*4 + [4]*4, [10]*4 + [4]*4), # 40+16=56
    ("TP8PP1_head_bias+4",  8, 1, [48],
     [3584] * 8, [11]*4 + [3]*4, [11]*4 + [3]*4), # 44+12=56
    # Hybrid FFN+50 + Head+3 (both Blackwell-biased)
    ("TP8PP1_hybrid_ffn50_head3",  8, 1, [48],
     [5376]*4 + [1792]*4, [10]*4 + [4]*4, [10]*4 + [4]*4),

    # ---- TP4 PP2: 48 layers = 24+24 uniform
    ("TP4PP2_layer_uniform_24-24", 4, 2, [24, 24],
     [7168] * 4, [14] * 4, [14] * 4),
    ("TP4PP2_layer_skew+4_28-20", 4, 2, [28, 20],
     [7168] * 4, [14] * 4, [14] * 4),
    ("TP4PP2_layer_skew+8_32-16", 4, 2, [32, 16],
     [7168] * 4, [14] * 4, [14] * 4),
    ("TP4PP2_layer_skew+12_36-12", 4, 2, [36, 12],
     [7168] * 4, [14] * 4, [14] * 4),
    ("TP4PP2_layer_skew+16_40-8", 4, 2, [40, 8],
     [7168] * 4, [14] * 4, [14] * 4),

    # ---- TP2 PP4: 48/4 = 12 per stage uniform
    ("TP2PP4_layer_uniform_12-12-12-12", 2, 4, [12, 12, 12, 12],
     [14336] * 2, [28] * 2, [28] * 2),
    ("TP2PP4_layer_blackbias_14-14-10-10", 2, 4, [14, 14, 10, 10],
     [14336] * 2, [28] * 2, [28] * 2),
    ("TP2PP4_layer_blackbias_16-16-8-8", 2, 4, [16, 16, 8, 8],
     [14336] * 2, [28] * 2, [28] * 2),

    # ---- TP1 PP8: 48/8 = 6 per stage uniform
    ("TP1PP8_layer_uniform_6x8", 1, 8, [6]*8,
     [28672], [56], [56]),
    ("TP1PP8_layer_blackbias_7-7-7-7-5-5-5-5", 1, 8, [7,7,7,7,5,5,5,5],
     [28672], [56], [56]),
    ("TP1PP8_layer_blackbias_8-8-8-8-4-4-4-4", 1, 8, [8,8,8,8,4,4,4,4],
     [28672], [56], [56]),
]

# OPT-30B is MHA (56 q-heads, 56 kv-heads) → KV cache ~4× larger per token
# than Llama-70B GQA (8 kv). n_req=128 would OOM on Ada at prefill_heavy
# (~193 GB cluster KV pressure). Halve concurrency to 64.
WORKLOADS = {
    "balanced":      (512, 256, 64),
    "decode_heavy":  (128, 512, 64),
    "prefill_heavy": (1024, 128, 64),
}
...
def launch_vllm(port, tp, pp, env, log_path, max_num_seqs=128):
    cmd = [
        ...
        # OPT-30B is a base model with no chat template; provide a minimal one
        # so the perf tool (which uses /v1/chat/completions) works.
        "--chat-template", "/tmp/opt_chat_template.jinja",
    ]
...
def main():
    ...
    out_root = REPO / "results" / f"hetero_4x4_opt30b_full_{ts}"
    ...
    for label, tp, pp, layer_split, ffn, head, kv in cfgs:
        for wl in wls:
            ...
            proc = launch_vllm(port, tp, pp, env, log_path, max_num_seqs=64)
